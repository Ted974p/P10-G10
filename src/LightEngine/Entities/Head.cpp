...
void Head::updateCameraWithDeadzones()
{
...
void Head::onDownCollision(Entity* other)
{
...
void Head::onCollisionEnter(Entity* other)
{
...
void Head::onCollisionExit(Entity* other)
{
	if (other->isTag(TAG::Player))
	{
		Demorfing = false;
...
bool Head::SetStates(State State)
{
...
void Head::MoveRight(float deltaTime)
{
	if (isMovingLeft)
	{
		mDeceleration = 180.f;
...
void Head::MoveLeft(float deltaTime)
{
	if (isMovingRight)
	{
		mDeceleration = 180.f;
...
void Head::Decelerate(float deltaTime)
{
...
void Head::setInLightEntity(bool value)
{
	if (value)
	{
...
void Head::onUpdate()
{
	...
	if (Demorfing)
	{
		...
		{
			//std::cout << "IIIIIIIIIIIIIIIIIIIIIIIII
			//PlayerBody->DeMorphing();
...
void Head::Set(PlayerEntity* player)
{
	PlayerBody = player;
	int x = PlayerBody->getPosition().x + 64;
	int y = PlayerBody->getPosition().y;
	setPosition(x, y);
}
